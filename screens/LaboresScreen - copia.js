...
// --- IMPORTACIONES FIREBASE CORREGIDAS ---
...
export default function LaboresScreen({ route }) {
  ...
  const cargarDatosBasicos = async () => {
    try {
      setLoading(true);
      // Inicializamos Realtime Database explícitamente
      ...
      if (snapshot.exists()) {
        ...
        console.log("✅ Datos encontrados:", datos ? "Sí" : "No");
        ...
        console.log("⚠️ No hay datos en la ruta:", ruta);
        // Si no hay en firebase, intentamos caché o local como fallback
        const datosGuardados = await AsyncStorage.getItem(CACHE_KEY);
        if (datosGuardados) {
            const parsedData = JSON.parse(datosGuardados);
            setCultivoData(parsedData);
            setNivel('basico'); // Asumimos básico si viene de caché antigua
        } else {
            // Último recurso: JSON local
  ...
  // --- FUNCIÓN DE FIREBASE CORREGIDA (REALTIME DATABASE) ---
  const obtenerDatosFirebase = async () => {
    try {
      ...
      // Hacemos la consulta usando 'get' y 'child'
      const snapshot = await get(child(dbRef, ruta));
      ...
      if (snapshot.exists()) {
        ...
        return {
          ...data,
          // Mapeo forzoso para asegurar compatibilidad
