...
export default function PlagasScreen({ route }) {
  ...
  const descargarDatosCompletos = async (fromRefresh = false) => {
    try {
      ...
      if (datosCompletos) {
        if (datosCompletos.plagas_y_enfermedades || datosCompletos.grados_dia_desarrollo || datosCompletos.riesgos_detallados || datosCompletos.plagas_detalladas || datosCompletos.plagas) {
    ...
    const listaNormalizada = rawList.map(item => {
        ...
        if (typeof condiciones === 'object' && condiciones !== null) {
            ...
            if (condiciones.humedad_relativa) {
                ...
                partes.push(`HR: ${h.minima || ''}-${h.optima || h.maxima || ''}%`);
            }
  ...
  const abrirEditor = (item, index) => {
    ...
    if (item.manejo_integrado?.general) {
        quimicoCombinado = quimicoCombinado ? `${quimicoCombinado}\n\n${item.manejo_integrado.general}` : item.manejo_integrado.general;
    }
    setEditItem({
        ...item,
        sintomasTexto: Array.isArray(item.sintomas) ? item.sintomas.join('\n') : item.sintomas,
        manejoQuimicoTexto: quimicoCombinado,
                          ...
                          {!!((item.control_quimico && item.control_quimico.length > 0 && item.control_quimico[0].ingrediente_activo) || 
                            ...
                            <View style={styles.managementRow}>
                              ...
                              <Text style={styles.managementText}>
                                {item.control?.productos_activos_mexico ? item.control.productos_activos_mexico[0].ingrediente : item.control_quimico[0].ingrediente_activo}
                              </Text>
                            </View>
                          )}
                        </View>
